...
export async function fetchSiteMetadata(target: NormalizedTarget): Promise<SiteMetadata> {
  ...
  try {
    const response = await fetch(target.url, { redirect: "follow", signal: controller.signal, headers: { "user-agent": "AnuncioTopBot/1.0 (+https://anuncio.top)", accept: "text/html" } });
