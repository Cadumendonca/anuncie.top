...
export async function POST(request: Request) {
  try {
    ...
    const preference = await new Preference(client).create({ body: { items: [{ id: order.id, title: `Posição no Seu Site em Alta`, quantity: 1, currency_id: "BRL", unit_price: chargeCents / 100 }], payer: { email: input.email }, external_reference: order.id, back_urls: { success: `${appUrl}/pagamento/sucesso`, pending: `${appUrl}/pagamento/pendente`, failure: `${appUrl}/pagamento/falha` }, auto_return: "approved", notification_url: `${appUrl}/api/webhooks/mercado-pago`, statement_descriptor: "SITEEMALTA" } });
